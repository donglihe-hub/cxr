...
class CXRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image = Image.fromarray(pixel_array(self.images[idx])).convert("RGB")
...
class CrossValidationCXRBinaryDataModule(L.LightningDataModule):
    def __init__(
        self,
        file_extension: str,
        data_dir: str,
        ...
        self.kf = KFold(n_splits=num_splits, shuffle=True, random_state=split_seed)
    ...
    def setup(self, stage=None):
        ...
        if self.file_extension in ["npy", "npz"]:
            self.dataset = EmbeddingDataset(embeddings, labels)
            self.splits = [k for k in self.kf.split(self.dataset)]
        else:
            images = data_paths
            self.dataset = CXRDataset(images, labels, transform=self.test_transform)
            self.splits = [k for k in self.kf.split(self.dataset)]
    ...
    def update_split(self, k):
        train_indexes, val_indexes = self.splits[k]
        train_indexes, val_indexes = torch.tensor(np.asarray(train_indexes.tolist())), torch.tensor(np.asarray(val_indexes.tolist()))
        self.train_dataset, self.val_dataset = Subset(self.dataset, train_indexes), Subset(self.dataset, val_indexes)
